...
class DatabaseHelper:
    def __init__(self, database_type='sqlite', db_path=None):
        if db_path is None:
            db_path = path.join(RESULTS_DIR, 'data.db')

        self.engine = sqlalchemy.create_engine('{}://{}'.format(database_type, db_path))
    ...
    def commit_result_data_to_sql(self, df_resource, table_name='data_result'):
        df_resource.to_sql(table_name, con=self.engine, schema=None, if_exists='append', index=False,
                           index_label=None)
    engine = None
    df_gen = None
